...
object MostPopularSuperhero {
  ...
  // First entry is for the hero ID and all subsequent entries are the heroes connected to that ID.
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("./src/main/scala/data/Marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)

    // Load up the superhero co-apperarance data
    val lines = sc.textFile("./src/main/scala/data/Marvel-graph.txt")
    ...
    println("10 Least famous characters are: ")
    flipped.sortByKey()
      .take(10)
      .foreach(character => println(s"${namesRdd.lookup(character._2)(0)} has ${character._1} appearances"))
    println()
    println("10 Most famous characters are: ")
    flipped.sortByKey(false)
      .take(10)
      .foreach(character => println(s"${namesRdd.lookup(character._2)(0)} has ${character._1} appearances"))
